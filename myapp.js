...
const rainfall = "https://data.weather.gov.hk/weatherAPI/opendata/weather.php?dataType=rhrread&lang=en";
const warning = "https://data.weather.gov.hk/weatherAPI/opendata/weather.php?dataType=warningInfo&lang=en"
...
async function getObData () {
 ...
 //A function to get warning
async function getwarning () {
 let response = await fetch(warning);
 return await response.json();
}
...
// A function to get 9-Day forecast data
async function getRfall () {
  let response = await fetch(rainfall);
  return await response.json();
 }
...
async function main () {
  ...
  try {
    var Rfall = await getRfall(); //wait for rainfall data
    generaterainfall(Rfall);
  ...
  try {
    var wa = await getwarning(); //wait for rainfall data
...
function generaterainfall(rainfall) {
  //get the HTML div element for this block
  let showR = document.getElementById('rainfall');
  //Create a div element to display the title
  let ttitle = document.createElement('div');
  ttitle.id = 'ttitlee';
  ttitle.innerHTML = 'Rainfall';
 showR.append(ttitle); //add the element to the block
//Read and clone the location temp dataset
let myRain = JSON.parse(JSON.stringify(rainfall.rainfall.data));
...
return 0;
  ...
  let RainData = document.createElement('div');
  RainData.id = 'sblock';
  //add each location as a selection option
  let toptions = '';
  myRain.forEach((elm, idx) => {
    toptions += `<option value="${idx}">${elm.place}</option>`
  });
  //create the label and the list of options
  let Rselect = '<label>Select the location</label><select name="rain" id="rain-reg">'+toptions+'</select>'
  RainData.innerHTML = Rselect;
  showR.append(RainData);  //add to the block
  ...
  document.getElementById('rain-reg').addEventListener('change', (evt) => {
    //according to current user's selection, retrieve the data
    let max = myRain[evt.target.value].max;
    //output the temp data to the 'tblock' div element
    document.getElementById('Rblock').innerHTML = `<span class='ltemp'>${max}</span><span class="sup2">mm</span>`
  });
...
FCblock.append(Ftitle); //add to displa
  ...
  for (let day of WFore.weatherForecast) {  //for each forecast day
    ...
    FCWrap.append(forecast);   //output all data and add to the containing div block
    forecast.innerHTML = fweek + fIcon + PSR + ftemp + fhumid;
